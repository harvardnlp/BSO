...
#        s = s.replace(self.UNK, "")
    ...
    def freq_prune_vocab(self, k):
        """ just keeps stuff occurring at least k times"""
        vocab_list = [(word, count) for word, count in self.vocab.iteritems()]
        vocab_list.sort(key = lambda x: x[1], reverse=True)
        #k = min(k, len(vocab_list))
        self.pruned_vocab = {pair[0]:pair[1]
          for pair in vocab_list
            if pair[1] >= k
              or (args.for_dep_parsing and (pair[0].startswith("@L_") or pair[0].startswith("@R_")))}
        for word in self.pruned_vocab:
            if word not in self.d:
                self.d[word] = len(self.d) + 1
...
def get_data(args):
    ...
    def make_vocab(srcfile, targetfile, seqlength, max_word_l=0, chars=0, add_to_vocab=True):
        num_sents = 0
    ...
    def convert(srcfile, targetfile, batchsize, seqlength, outfile, num_sents,
        ...
        #print "newseqlength is", newseqlength
    ...
    max_word_l, num_sents_train = make_vocab(args.srcfile, args.targetfile,
                                             args.seqlength, 0, args.chars, add_to_vocab=True)
    ...
    max_word_l, num_sents_valid = make_vocab(args.srcvalfile, args.targetvalfile,
                                             args.seqlength, max_word_l, args.chars, add_to_vocab=False)
    ...
    if args.srcminfreq > 0:
        print "pruning things occurring <", args.srcminfreq, "times"
        src_indexer.freq_prune_vocab(args.srcminfreq)
        target_indexer.freq_prune_vocab(args.srcminfreq)
    else:
...
def main(arguments):
    ...
    parser.add_argument('--srcminfreq', help="Only keep src tokens occurring at least this many times",
                                       type = int, default = 0)
    parser.add_argument('--for_dep_parsing', dest='for_dep_parsing', action='store_true')
