-- A slightly modified version of https://github.com/facebookresearch/MIXER/blob/master/tokenizer.lua which is to used to allow outputting text rather than torch Tensors.
...
local function just_write_words(sent, dict, outfi)
    ...
    for i, word in pairs(words) do
        if word ~= "" then
            ...
            local wid = dict.symbol_to_index[word]
            if wid == nil then
              outfi:write(dict.index_to_symbol[unk_idx])
            else
              --outfi:write(word)
              outfi:write(dict.index_to_symbol[wid])
            end
        end
    end
    outfi:write('\n')
